...
async function sendSms(to, message, fromNumber) {
  ...
  try {
    ...
    else console.log(`[SMS SENT] to ${to} from ${fromNumber}`);
  } catch (err) {
...
async function sendCheckoutReminders() {
  try {
    ...
    for (const booking of res.rows) {
      ...
      setTimeout(async () => {
        await sendSms(guest.rows[0].phone,
          `Hope you enjoyed your stay! If you have a moment we'd love a review. Thank you!`,
...
function buildSystemPrompt(property, booking) {
  return `You are an AI host assistant for a short-term rental. You help guests via SMS. Keep replies SHORT (1-3 sentences max — this is SMS).
...
async function processGuestMessage(host, guestPhone, message, property, booking) {
  try {
    ...
    if (cleanReply) await sendSms(guestPhone, cleanReply, host.twilio_number);
    ...
    await sendSms(guestPhone, "Sorry, something went wrong. The host will reach out directly.", host.twilio_number);
  }
}
...
async function handleHostReply(host, body) {
  ...
  `, [host.id]);
  if (!res.rows[0]) {
    await sendSms(host.host_phone, "No open escalations.", host.twilio_number);
    return;
  }
  const guestPhone = res.rows[0].guest_phone;
  await sendSms(guestPhone, body, host.twilio_number);
  await pool.query("UPDATE conversations SET escalated = false WHERE guest_phone = $1 AND host_id = $2",
    [guestPhone, host.id]);
  await sendSms(host.host_phone, `Forwarded to guest at ${guestPhone}.`, host.twilio_number);
}
...
async function handleSms(host, from, body) {
  ...
  if (guest.onboarding_state === "awaiting_address") {
    ...
    await sendSms(from, `Just to confirm — are you staying at ${property.address}? Reply YES or NO.`, host.twilio_number);
    ...
    setTimeout(async () => {
      try {
        ...
        if (g?.onboarding_state === "awaiting_confirmation") {
          await resetGuest(from, host.id);
          await sendSms(from, `We didn't hear back. Just text the address whenever you're ready!`, host.twilio_number);
        }
  ...
  if (guest.onboarding_state === "awaiting_confirmation") {
    ...
    if (["yes", "yeah", "yep", "yup", "correct", "right", "y", "ya"].includes(answer)) {
      ...
      await sendSms(from,
        `Perfect, you're all set! I'm your AI host assistant for ${property.address}. Ask me anything — WiFi, checkout info, local spots, or if something needs fixing. I'm here 24/7!`,
        host.twilio_number);
      console.log(`[LINKED] ${from} → property ${guest.pending_property_id}`);
    } else if (["no", "nope", "n", "wrong", "incorrect"].includes(answer)) {
      await resetGuest(from, host.id);
      await sendSms(from, `No problem! What's the address you're staying at?`, host.twilio_number);
    } else {
      await sendSms(from, `Sorry, I didn't catch that. Reply YES or NO.`, host.twilio_number);
    }
    return;
  }

  await sendSms(from, `Hi! What's the address of the property you're staying at?`, host.twilio_number);
}
...
app.get("/dashboard", checkHostAuth, async (req, res) => {
  ...
  const activeRows = activeBookings.rows.map(b => `<tr><td>${b.property_name || b.address}</td><td>${b.checkin_date}</td><td>${b.checkout_date}</td><td>${b.guest_name || "—"}</td><td><span style="background:#22c55e;color:#fff;padding:2px 8px;border-radius:12px;font-size:12px;font-weight:bold">${b.status}</span></td></tr>`).join("");
  const escalationRows = escalations.rows.map(e => `<tr><td>${e.address || "Unknown"}</td><td>${e.guest_phone}</td><td>${timeAgo(e.updated_at)}</td><td><span style="background:#ef4444;color:#fff;padding:2px 8px;border-radius:12px;font-size:12px;font-weight:bold">Open</span></td></tr>`).join("");
  const propRows = properties.map(p => `<tr><td>${p.name || p.address}</td><td style="font-size:12px;color:#64748b">${p.address}</td><td>${p.ical_url ? '<span style="color:#22c55e;font-weight:bold">Yes</span>' : '<span style="color:#94a3b8">No</span>'}</td><td style="display:flex;gap:6px"><a href="/properties/${p.id}/edit" style="background:#3b82f6;color:white;padding:4px 12px;border-radius:6px;font-size:12px;font-weight:bold;text-decoration:none">Edit</a><form method="POST" action="/properties/${p.id}/sync" style="display:inline"><button type="submit" style="background:#8b5cf6;color:white;border:none;padding:4px 12px;border-radius:6px;font-size:12px;font-weight:bold;cursor:pointer">Sync iCal</button></form></td></tr>`).join("");
...
app.post("/admin/hosts", checkAdminAuth, async (req, res) => {
  ...
  await pool.query(`INSERT INTO hosts (name, email, twilio_number, host_phone, dashboard_password, plan) VALUES ($1, $2, $3, $4, $5, $6)`,
...
app.get("/privacy", (req, res) => {
  res.send(`<html><head><title>Privacy</title><style>body{font-family:Arial,sans-serif;background:#f5f7fb;padding:40px;max-width:900px;margin:auto;line-height:1.7;}h1{font-size:36px;margin-bottom:20px;}h2{font-size:24px;margin-top:30px;}p{font-size:18px;}</style></head><body><h1>Privacy Policy</h1><p>${BRAND} collects phone numbers and message content to facilitate guest-host communication.</p><h2>Information We Collect</h2><p>Phone numbers, property addresses, message content, booking dates.</p><h2>How We Use It</h2><p>Solely for guest support during stays.</p><h2>Opt-Out</h2><p>Reply STOP at any time.</p></body></html>`);
});

app.get("/terms", (req, res) => {
  res.send(`<html><head><title>Terms</title><style>body{font-family:Arial,sans-serif;background:#f5f7fb;padding:40px;max-width:900px;margin:auto;line-height:1.7;}h1{font-size:36px;margin-bottom:20px;}p{font-size:18px;}</style></head><body><h1>Terms of Service</h1><p>By using ${BRAND}, you consent to SMS messages related to your stay. Msg and data rates may apply. Reply STOP to opt out.</p></body></html>`);
});
