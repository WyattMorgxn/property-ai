...
app.post("/onboarding/submit", async (req, res) => {
  try {
    const { name, email, phone, password, confirmPassword, propertyName, address, platform, icalUrl,
      ...
      nearestGrocery, localRestaurants, nearestHospital, otherTips, emergencyContact } = req.body;
    ...
    const hostRes = await pool.query(
      `INSERT INTO hosts (name, email, host_phone, dashboard_password, plan) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [name.trim(), email.trim().toLowerCase(), phone.trim(), password, 'starter']
    );
