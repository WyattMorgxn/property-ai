...
app.post("/onboarding/submit", async (req, res) => {
  try {
    ...
    const hostRes = await pool.query(
      ...
      [name.trim(), email.trim().toLowerCase(), phone.trim(), password, (twilioNumber||'').trim(), 'starter']
    );
...
// ── CONVERSATIONS VIEW ────────────────────────────────────────────
app.get("/conversations", checkHostAuth, async (req, res) => {
  try {
    ...
    res.send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Tenario - Conversations</title>'
      ...
      + '<div class="topbar"><div class="logo">Tenario</div><nav class="nav"><a href="/dashboard">Dashboard</a><a href="/conversations" style="color:#10b981;font-weight:700">Conversations</a><a href="/logout">Sign Out</a></nav></div>'
